...
pub async fn logout_handler(user_uuid: Uuid) -> Result<Response, Rejection> {  //  Принимаем user_uuid вместо session_id
    ...
    resp.headers_mut().insert(
        "Set-Cookie",
        format!("session_id=; Max-Age=0; Path=/; SameSite=Strict").parse().unwrap()
    );
