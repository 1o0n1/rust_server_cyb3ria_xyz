...
async fn main() {
    ...
    let chat_route = warp::path("api")
        ...
        .map(
            move |ws: warp::ws::Ws, _addr: Option<std::net::SocketAddr>, params: std::collections::HashMap<String, String>| {
                let clients_clone = Arc::clone(&clients_clone);
                let sender_clone = Arc::clone(&sender_clone);
                let session_id = params.get("session_id").map(|s| s.to_string());
                ws.on_upgrade(move |socket| {
                    client_connection(socket, clients_clone, sender_clone, session_id)
                })
            },
        )
        .boxed();
